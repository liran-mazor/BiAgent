...
export class Agent {
  ...
  private readonly patternHandlers: Record<string, (messages: any[], formattedTools: any[], conversationId: string) => Promise<string>> = {
  ...
  public async run(
    ...
    return await handler(messages, formattedTools, conversationId);
  }
  ...
  private formatToolsForClaude() {
    ...
    const allTools = [...nativeToolsFormatted, ...a2aToolsFormatted];
  ...
  private async executeTool(toolUseBlock: Anthropic.ToolUseBlock): Promise<{ tool_use_id: string; result: any }> {
    ...
    throw new Error(`Tool ${toolUseBlock.name} not found in native or A2A tools`);
  }
