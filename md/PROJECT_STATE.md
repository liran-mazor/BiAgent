...
**Name:** BiAgent
**Architecture:** BiAgent orchestrator (interface layer) + 2 A2A agents (knowledge-agent, analytics) + API gateway + 5 platform microservices
**Tools:** `chart`, `forecast_revenue`, `email`, `web_search` (native), `query_analytics` (A2A → analytics → ClickHouse), `query_knowledge` (A2A → knowledge-agent → pgvector)
**Interfaces:** CLI, interactive CLI, Telegram bot, Alfred (RPi voice)
**Monorepo:** npm workspaces — `knowledge-agent/`, `app/{analytics,gateway,common,infra,orders,catalog,customers,reviews,backoffice}`
