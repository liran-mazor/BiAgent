...
import { executeChart } from './chartTool.js';
import { executeForecast } from './forecastTool.js';
import { z } from 'zod';
...
const taskMap: Record<string, Function> = {
  chart: executeChart,
  forecast_revenue: executeForecast
};
...
const PORT = 3003;
app.listen(PORT, () => {
  console.log(`📊 AnalyticsAgent running on http://localhost:${PORT}`);
});
